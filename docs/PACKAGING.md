...
The unscoped `fluxus` name belongs to another project. The manifest uses `@othmaneblial/fluxus`; version `0.1.0` is published under the authenticated `othmaneblial` account and was verified through the registry's version metadata and tarball URL. The current `0.1.1` candidate is not published yet.
...
There is no CLI or native binary to download. A versioned npm tarball is the distribution artifact. The `0.1.0` package and GitHub release archive are available and verified; use the release procedure to publish the `0.1.1` candidate and future versions.
