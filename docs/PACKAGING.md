...
The unscoped `fluxus` name belongs to another project. The manifest uses `@othmaneblial/fluxus`; version `0.1.0` is published under the authenticated `othmaneblial` account and was verified through the registry's version metadata and tarball URL. The release asset and registry tarball have the same SHA-256.
...
There is no CLI or native binary to download. A versioned npm tarball is the distribution artifact. The `0.1.0` package and GitHub release archive are available and verified; use the release procedure for future versions.
