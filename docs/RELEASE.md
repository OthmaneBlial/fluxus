...
This procedure prepares a local artifact and describes the external checks. Fluxus `0.1.1` is published and verified; `0.1.0` remains available as the first release. Future versions must repeat the same registry, tag, asset and public-demo checks. A `package.json` version alone is never proof of a release.
