...
This document began as a product decision against commit `5ab9472` on 19 September 2026. The core, types, tarball and browser workbench are implemented and checked locally; the published `0.1.1` release and public demo are verified, while independent usage is still an open gate.
...
- Package availability is not inferred from `package.json`: the unscoped npm name `fluxus` is occupied by an unrelated package. The manifest uses `@othmaneblial/fluxus`; `0.1.1` is verified through the npm registry and its release tarball.
