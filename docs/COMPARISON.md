...
This is a comparison of **design and available APIs**, not a speed, memory, bundle-size, or adoption ranking. The unscoped npm package named `fluxus` is an unrelated project; this repository's manifest version is not evidence of its publication.
...
No general performance winner is named here. [The local benchmark](BENCHMARKS.md) now records pinned versions, correctness checks, repeated samples, bundle sizes and a limited memory profile for specific tasks. Its workload and feature differences are documented there; the numbers are not a ranking of complete applications.
