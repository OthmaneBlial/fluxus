...
After that publication, the repository README was updated to link directly to the verified package, release and public workbench. The current `main` source therefore intentionally does not reproduce the already published `0.1.0` tarball byte-for-byte: the only archive difference is the README. The improved `yarn release:check` detects this mismatch and fails until the next source release receives a new version; this preserves the rule that a published artifact must correspond exactly to its validated commit.

## Published 0.1.1 release and refreshed public demo (19 September 2026)

The exact candidate from commit `5f65010` passed `NPM_TOKEN=unused yarn release:check`: 40 tests, lint, documentation links, type-check, build, the nine-file package consumer and npm dry-run. The generated `release/othmaneblial-fluxus-0.1.1.tgz` was 11,735 bytes with SHA-256 `1c72bc41cc336b8da9e0f0602397d46a358cd2c1602a2bf49b3a3cd2f62a81ad`.

`@othmaneblial/fluxus@0.1.1` was published from that exact archive. After registry propagation, the version metadata returned the package and tarball URL; the downloaded registry tarball had the same SHA-256. A fresh consumer installed the public specifier `@othmaneblial/fluxus@0.1.1` and loaded both CommonJS and ESM exports. The annotated tag `v0.1.1` dereferenced to `5f65010`; GitHub Release [v0.1.1](https://github.com/OthmaneBlial/fluxus/releases/tag/v0.1.1) is public with the tarball, `SHA256SUMS` and `release-manifest.json`.

Demo Pages run [35447224823](https://github.com/OthmaneBlial/fluxus/actions/runs/35447224823) succeeded for build and deploy from `v0.1.1). Chrome opened the public URL, added `Verify 0.1.1 public state`, completed it, filtered Open tasks and paused the independent observer. The rendered state reached 2 open / 2 done and 50% progress, and the console had no warnings or errors. The current `main` README updates after the tag are documentation-only; the release check will require a new version before another package publication.
