...
`social-preview.html` is the editable source of the cover. Its copy describes the existing demo and its inset is a crop of the real screenshot, not a product mockup. The cover was uploaded to the GitHub repository's Social preview setting on 19 September 2026; the settings page displayed the image after upload. The capture itself predates hosting; the current public demo is recorded in [validation evidence](../VALIDATION.md).
